...
class BasePlotConfig(BaseModel):
    ...
    layout: Literal['constrained', 'compressed', 'tight', 'none'] = Field(default='compressed')
...
class Timeseries(Plot):
    ...
    def plot(self):
        ...
        domains = {name: dom for name, dom in self.domains.items() if name in self.plot_config.domains}
        ...
        for dom_name, dom in domains.items():
            # Apply dom
            all_data_dom = {name: dom.apply(data) for name, data in all_data.items()} if dom is not None else all_data
            ...
            for variable in vars:
                ...
                filename = f"ts-{dom_name}-{variable}-{start.strftime("%d-%m-%Y")}_{end.strftime("%d-%m-%Y")}.{format}" if self.plot_config.filename is None else self.plot_config.filename
    ...
    def plot(self):
        ...
        for dom_name, dom in domains.items():
            ...
            for variable, unit in vars.items():
                ...
                start, end = manage_time_interval(time_interval)
                format = self.plot_kwargs.get("format", "jpg")
                filename = f"scatter-{dom_name}-{variable}-{start.strftime("%d-%m-%Y")}_{end.strftime("%d-%m-%Y")}.{format}" if self.plot_config.filename is None else self.plot_config.filename
                self.savefig(figure, filename)
    ...
    def plot(self):
        ...
        for dom_name, dom in domains.items():
            for var in vars:
                ...
                ax.contourf(base_var["longitude"].values, 
                ...
                start, end = manage_time_interval(time_interval)
                format = self.plot_kwargs.get("format", "jpg")
                filename = (f"spatial_overlay-{dom_name}-{var}-{base.name}-{superposed.name}-{start.strftime("%d-%m-%Y")}_{end.strftime("%d-%m-%Y")}.{format}" if self.plot_config.filename is None else self.plot_config.filename)
