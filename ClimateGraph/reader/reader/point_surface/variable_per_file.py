...
class VariablePerFileReader(CSVPointSurfaceReader):
    """Reader for the variable-per-file CSV layout (one variable per file).

    Layout: one CSV per variable, each with a time column and one column per
    station (station names as headers), plus a metadata sheet (CSV/Excel). The
    variable a file carries is NOT inferred from its name — the data block
    supplies an explicit ``var_files`` mapping ``{canonical_var: filename}``.
    Originally written for Quito (REMMAQ), whose defaults this keeps.

    Station names in the CSV headers (e.g. ``ELCAMAL``, ``GUAMANI``) may be the
    ASCII-uppercased, space-stripped forms of accented names in the metadata
    (``El Camal``, ``Guamaní``); ``_norm_site_key`` reconciles them.
    """

    type_aliases = ["variable-per-file"]
    ...
    def _var_for(cls, path: Path, spec: ReadSpec) -> str:
        ...
        if not var_files:
            raise ValueError(
                f"{cls.__name__} requires a 'var_files' mapping "
                "{canonical_var: filename} in the data block."
            )
        for canonical, filename in var_files.items():
            if Path(filename).name == path.name:
                return canonical
        raise ValueError(
            f"{cls.__name__}: file {path.name!r} not found in var_files {var_files}."
        )
